...
router.get("/fetch-students", async (req, res) => {
    ...
    try {
        ...
        res.json({ message: 'Data fetched and stored successfully!', data: processedData });
    } catch (error) {
...
const fetchAndProcessAllStudentData = async () => {
    try {
        ...
        const students = await databases.listDocuments
            (
                database_id, studentTable_id,
                [
                    Query.limit(10000)
                ]
            );

        return Promise.all(students.documents.map(async (student) => {
            const results = await databases.listDocuments
                (
                    database_id, studentMarksTable_id,
                    [
                        Query.equal("studID", [student.studID]),
                        Query.limit(50)
                    ]
                );
            ...
            const points = await databases.listDocuments
                (
                    database_id, pointsTable_id,
                    [
                        Query.equal("UserID", [student.studID]),
                        Query.limit(1)
                    ]
                );

            const pointsBalance = points.documents.length > 0 ? points.documents[0].PointsBalance : 0;
